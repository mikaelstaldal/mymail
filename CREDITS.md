...
Both libraries ship prebuilt and self-contained, and are vendored verbatim (no CDN, no bundler)
under `web/static/vendor/` by the maintainer script `web/ts/vendor/rebuild.sh`. Type stubs for
Preact are vendored under `web/ts/vendor/preact/`.
